...
def Modify_database():
    ...
    with st.container():
        ...
        input_parameters_df = pd.DataFrame(data, index=[0,1])
        st.markdown('**Updated cost data with user-defined cost value**')
        st.dataframe(input_parameters_df,use_container_width=True,hide_index=True)
...
def User_defined_building():
        ...
        if uploaded_file:
            ...
            for i in range(0, num_member ):
                ...
                if Thickness_method == 'Use thickness adjust equation':
                    ...
                    np.savetxt(file_name, member_price, delimiter=',', fmt='%d')
    ...
    with st.container():
        ...
        values = [total_fire_protection_cost_material1, total_fire_protection_cost_material2,
        ...
        bottom = 0
        bars = ax4.bar(categories, values, color=value_colors)
        # for bar, value in zip(bars, values):
        # ax.annotate(str(value), xy=(bar.get_width(), bar.get_y() + bar.get_height() / 2),
        #          xytext=(5, 0), textcoords='offset points', va='center')

        for value, color, label in zip(values, value_colors, categories):
